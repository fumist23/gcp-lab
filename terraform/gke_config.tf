resource "google_container_cluster" "config" {
  name     = "config-cluster"
...
resource "google_gke_hub_membership" "config" {
  provider = google-beta
  membership_id = "config"
  endpoint {
    gke_cluster {
      resource_link = "//container.googleapis.com/${google_container_cluster.config.id}"
    }
  }

  authority {
    issuer = "https://container.googleapis.com/v1/${google_container_cluster.config.id}"
  }
}
